...
describe("About Applying What We Have Learnt", function () {
  ...
  it("given I'm allergic to nuts and hate mushrooms, it should find a pizza I can eat (functional)", function () {
    ...
    productsICanEat = products.filter(product =>
      product.containsNuts === false && !product.ingredients.includes('mushrooms'));
  ...
  it("should add all the natural numbers below 1000 that are multiples of 3 or 5 (functional)", function () {
    ...
    var sum = (_.range(0, 1000).filter(function (x) {
      return x % 3 === 0 || x % 5 === 0;
    }).reduce((acc, x) =>
      acc + x));
    console.log(sum)
  ...
  it("should count the ingredient occurrence (functional)", function () {
    ...
    ingredientCount['mushrooms'] = products.map(product =>
      product.ingredients.includes('mushrooms')).reduce((sum, ingredient) => sum + ingredient);
